...
import { getPayloadApi } from "./payload.ts";
...
class TableEditorService {
  ...
  onVisibleColumnsChanges(columns: Field[]): void {
    ...
    this.sheetColumns = columns.map((column: any) => ({
  ...
  getChangeHandler(
    ...
    changeList: ChangeItem[],
    onChanges: (
      cellChanges: CellChange<TextCell>[],
      changeList: ChangeItem[]
    ) => void
    ...
    return (cellChanges: CellChange<TextCell>[]) => {
      const changes: { index: number; column: string }[] = cellChanges
        .filter(
          (change, index, self) =>
            index ===
            self.findIndex(
              (t) => t.rowId === change.rowId && t.columnId === change.columnId
            )
        )
        .map((change) => ({
      ...
      const originalObjects = changes.map((change) => ({
      ...
      changes.forEach((change, index) => {
        ...
        const existingChange = newObjectChanges.find(
          (change) => change.row["__index__"] === clone["__index__"]
        );

        if (records[change.index]["__hash__"] !== hash(JSON.stringify(clone))) {
          if (existingChange != null) {
            const listIndex: number = newObjectChanges
              .map((a) => a.row)
              .indexOf(existingChange.row);
            const columnIndex: number = newObjectChanges[listIndex].columns
              .map((a) => a.title)
              .indexOf(change.column);

            if (
              newObjectChanges[listIndex].columns[columnIndex]?.original ==
              clone[change.column]
            ) {
              newObjectChanges[listIndex].columns.splice(columnIndex, 1);
            } else if (columnIndex === -1) {
              newObjectChanges[listIndex].columns = [
                ...newObjectChanges[listIndex].columns,
                {
                  title: change.column,
          ...
          newObjectChanges = newObjectChanges.filter(
            (change) => change.row["__index__"] !== clone["__index__"]
          );
        }
      });
  ...
  getValueRows(
    ...
    return values.map((row, index: number) => {
      const change: ChangeItem | undefined = changes.find(
        (change) => change.row["__index__"] === index
      );
  ...
  getSheetValues(changes: ChangeItem[]): Row<Cell>[] {
    return [
      this.getHeaderRow(this.visibleColumns),
      ...this.getValueRows(this.visibleColumns, this.records, changes),
    ];
  }
  ...
  getValueRow(
    ...
    columns: any[],
    values: any[],
    change: ChangeItem | undefined
  ): Row<Cell> {
    return {
      rowId: index,
      cells: columns.map((field): Cell => {
        const columnChange = change?.columns.find(
          (column) => column.title === field.name
        );
...
export const AdjustmentPage = () => {
  ...
  const getData = () => {
    getPayloadApi()
      .then((res) => {
        const service = new TableEditorService(
          res.payload.fields,
          res.payload.records
        );
        setService(service);
        setColumns(res.payload.fields);
      })
      .catch((err) => {
  ...
  return (
    <>
      ...
      <textarea value={"WHERE CIFNUM = '1111111111'"}></textarea>
      ...
      {service?.records?.length != undefined &&
